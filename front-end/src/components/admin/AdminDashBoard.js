...
const Dashboard = () => {
  ...
  const fetchProducts = async () => {
    try {
      const appleResponse = await axios.get('http://yugopro.com/api/products/Apple');
  ...
  const handleFormSubmit = async (product) => {
    ...
    if (editingProduct) {
      ...
      try {
        ...
        await axios.put(`http://yugopro.com/api/products/${product.category}/${editingProduct._id}`, product);
      ...
      // Create new product
      try {
        const response = await axios.post(`http://yugopro.com/api/products/${product.category}`, product);
  ...
  const handleDelete = async (singleProduct) => {
    try {
      await axios.delete(`http://yugopro.com/api/products/${singleProduct.category}/${singleProduct._id}`);
