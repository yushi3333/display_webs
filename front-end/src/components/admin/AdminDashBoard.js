...
const Dashboard = () => {
  ...
  const handleFormSubmit = async (product) => {
    ...
    if (editingProduct) {
      ...
      try {
        ...
        await axios.put(`http://127.0.0.1:3002/api/products/${product.category}/${editingProduct._id}`, product);
      ...
      // Create new product
      try {
        const response = await axios.post(`http://127.0.0.1:3002/api/products/${product.category}`, product);
  ...
  const handleDelete = async (singleProduct) => {
    try {
      await axios.delete(`http://127.0.0.1:3002/api/products/${singleProduct.category}/${singleProduct._id}`);
